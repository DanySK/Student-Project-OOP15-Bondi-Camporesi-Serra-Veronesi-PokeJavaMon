...
//    private final ActionListener fightListener;
//    private final ActionListener bagListener;
//    private final ActionListener squadListener;
//    private final ActionListener runListener;
...
//				View.getView().hideCurrent();
	...
	private void set4MovesToButtons() {
		final List<Move> moves = ally.getCurrentMoves();
		int count = 0;
	...
	public void refresh() {
		...
		if(!currentEnemy.equals(this.enemy)) {
		    ...
		    this.enemyFrontSprite.setIcon(new ImageIcon(currentEnemy.getPokedexEntry().getFrontSprite().getResourcePath()));
		}
		
		this.enemyHealthBar.setValue(this.enemy.getCurrentHP());
		
		if(!currentAlly.equals(this.ally)) {
		    ...
		    this.allyBackSprite.setIcon(new ImageIcon(currentAlly.getPokedexEntry().getFrontSprite().getResourcePath()));
		    this.allyExpBar.setValue(currentAlly.getStat(Stat.EXP) - currentAlly.getNecessaryExp());
		}
		this.allyHealthBar.setValue(this.ally.getCurrentHP());
		if (this.allyExpBar.getValue() != currentAlly.getStat(Stat.EXP)){
			System.out.println("Modifying currentEXP");
...
//		f.getContentPane().add(new HealthBar(StaticPokemonFactory.createPokemon(Pokedex.ABRA, 10), StaticPokemonFactory.createPokemon(Pokedex.CHARMANDER, 12)));
//		f.setVisible(true);
//	}
}
